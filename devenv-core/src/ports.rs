    ...
    pub fn allocate(&self, process_name: &str, port_name: &str, base: u16) -> Result<u16, String> {
        ...
        if strict {
            ...
            if allocated_ports.contains(&base) {
                return Err(format!(
                    "Port {} is already allocated to another process in this devenv session",
                    base
                ));
            }
            ...
            match TcpListener::bind(&addr) {
                ...
                Err(_) => {
                    ...
                    return Err(format!(
                        "Port {} is already in use{}. Use --strict-ports=false to auto-allocate an available port.",
                        base, process_info
                    ));
                }
            }
...
mod tests {
    ...
    fn test_port_allocator_skips_already_allocated() {
        ...
        // Second allocation for different process from same base should get next port
