...

{
  ...
  enterTest =
    ...
    ''
      ...
      # Accept either "already in use" (external process) or "already allocated" (same session)
      if echo "$output" | grep -qE "already in use|already allocated"; then
        echo "✓ --strict-ports correctly failed with port conflict error"
      else
        echo "✗ Expected error message to contain 'already in use' or 'already allocated', got: $output"
        exit 1
      fi
    '';
}
