...
impl Devenv {
    ...
    pub async fn test(&self) -> Result<()> {
        ...
        // Use .status() instead of .output() to stream test output directly to terminal
        let mut shell_cmd = self.prepare_shell(&Some(test_script), &[]).await?;
        let status = shell_cmd.status().await.into_diagnostic()?;
